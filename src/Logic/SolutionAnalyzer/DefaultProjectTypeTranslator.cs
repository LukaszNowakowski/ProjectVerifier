...
public class DefaultProjectTypeTranslator : IProjectTypeTranslator
{
    private static readonly ReadOnlyDictionary<Guid, string> TypeNames = new ReadOnlyDictionary<Guid, string>(
    ...
    public string? GetProjectTypeName(Guid projectTypeId)
    {
        if (!TypeNames.ContainsKey(projectTypeId))
        {
            return null;
        }

        return TypeNames[projectTypeId];
    }
}